...
namespace Utils {
	...
	inline std::mutex PlayerListMutex; // Thread-safe access to PlayerList
	...
	inline bool World2Screen(Unity::Vector3 world, Vector2& screen)
	{
		try
		{
			...
			if (buffer.x > Variables::ScreenSize.x || buffer.y > Variables::ScreenSize.y || buffer.x < 0 || buffer.y < 0 || buffer.z < 0)
			{
				return false;
			}
		...
		{
			// Silently fail if World2Screen crashes
			return false;
		}
	}
